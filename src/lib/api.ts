...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'https://api.thepitai.com/v1';
...
class ApiClient {
  ...
  setApiKey(key: string | null) {
    ...
    if (key && typeof window !== 'undefined') {
      localStorage.setItem('thepit_api_key', key);
    }
  }
  ...
  getApiKey(): string | null {
    ...
    if (typeof window !== 'undefined') {
      this.apiKey = localStorage.getItem('thepit_api_key');
    }
    return this.apiKey;
  }
  ...
  clearApiKey() {
    this.apiKey = null;
    if (typeof window !== 'undefined') {
      localStorage.removeItem('thepit_api_key');
    }
  }
