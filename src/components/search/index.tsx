...
export function SearchModal() {
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('thepit_recent_searches');
  ...
  const saveSearch = (term: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('thepit_recent_searches', JSON.stringify(updated));
    }
  };
  ...
  const clearRecent = () => {
    setRecentSearches([]);
    if (typeof window !== 'undefined') {
      localStorage.removeItem('thepit_recent_searches');
    }
  };
